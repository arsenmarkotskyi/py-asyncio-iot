...
class IOTService:
    ...
    async def run_program(self, program: list[Message]) -> None:
        ...
        device_programs = defaultdict(list)
        for msg in program:
            device_programs[msg.device_id].append(msg)
        print("=====END OF PROGRAM======")
        await asyncio.gather(
            *(self._run_device_program(device_id, msgs) for device_id, msgs in device_programs.items())
        )

    async def _run_device_program(self, device_id: str, messages: list[Message]) -> None:
        for msg in messages:
